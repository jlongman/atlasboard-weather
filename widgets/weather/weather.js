  ...
  onData: function (el, data) {
    ...
    if (data.hourlyContent && data.hourlyContent.list && data.hourlyContent.list.length > 0) {
      ...
      data.hourlyContent.list.forEach(function (weatherItem) {
        ...
        var td3 = $('<tr></tr>').append('<td>' + Math.round(weatherItem.main.temp) + '&deg;</td>').appendTo(hourlyTable);
        var td4 = $('<tr></tr>').append('<td>' + weatherItem.weather[0].main + '</td>').appendTo(hourlyTable);
      });
    }
